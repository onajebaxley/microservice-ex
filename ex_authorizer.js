...
exports.handler = function(event, context, callback) {
    ...
    policy.denyMethod(AuthPolicy.HttpVerb.GET, '*');
    policy.denyMethod(AuthPolicy.HttpVerb.PUT, '*');
    policy.denyMethod(AuthPolicy.HttpVerb.DELETE, '*');

    if (event.authorizationToken === 'someuser:somepass') {
        policy.allowMethod(AuthPolicy.HttpVerb.POST, "/");
    } else {
        policy.denyMethod(AuthPolicy.HttpVerb.POST, '*');
    }
    ...
    authResponse.context = {
        keyy : 'valuee', // $context.authorizer.key -> value
        number : 1,
        bool: true
    };
